...
import argparse
from utils import evaluation_batch_loco, global_cosine_hm_percent, global_cosine, \
    regional_cosine_hm, WarmCosineScheduler,visualize_one
...
def train(item_list):
    ...
    for i, item in enumerate(item_list):
        ...
        test_data = LOCODataset(root=test_path, transform=data_transform, gt_transform=gt_transform, phase="test")
    ...
    for epoch in range(int(np.ceil(total_iters / len(train_dataloader)))):
        ...
        for img, label in train_dataloader:
            ...
            if (it + 1) % 5000 == 0:
                torch.save(model.state_dict(), os.path.join(args.save_dir, args.save_name, f'model_{encoder_name}_{it+1}.pth'))
                ...
                for item, test_data in zip(item_list, test_data_list):
                    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=False,
                                                                  num_workers=4)
                    results = evaluation_batch_loco(model, test_dataloader, device, max_ratio=0.01)
                    auroc_sp, ap_sp, f1_sp, auroc_px, ap_px, f1_px= results
                    ...
                    print_fn(
                        '{}: I-Auroc:{:.4f}, I-AP:{:.4f}, I-F1:{:.4f}, P-AUROC:{:.4f}, P-AP:{:.4f}, P-F1:{:.4f}'.format(
                            item, auroc_sp, ap_sp, f1_sp, auroc_px, ap_px, f1_px))

                print_fn(
                    'Mean: I-Auroc:{:.4f}, I-AP:{:.4f}, I-F1:{:.4f}, P-AUROC:{:.4f}, P-AP:{:.4f}, P-F1:{:.4f}'.format(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='../../../../dataset/MMAD/MVTec-LOCO')
    parser.add_argument('--save_dir', type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str,
                        default='vitill_LOCO_uni_dinov2br_c392r_en29_bn4dp2_de8_laelu_md2_i1_it10k_sams2e3_wd1e4_w1hcosa_ghmp09f01w01_b16_ev_s1')
